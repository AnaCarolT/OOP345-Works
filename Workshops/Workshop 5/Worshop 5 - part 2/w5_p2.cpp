...
enum AppErrors
{
	CannotOpentxt = 1, // An input txt cannot be opened
...
// ws books.txt movies.txt txt_missing.txt txt_words.txt
int main(int argc, char** argv) {
	...
	{
		// TODO: load the first 4 books from the txt "argv[1]".
		// - read one line at a time, and pass it to the Book constructor
		// - store each book read into the collection "library" (use the+= operator)
		// - lines that start with "#" are considered comments and shouldbe ignored
		std::ifstream txt(argv[1]);
		std::string text;
		size_t m_counter = 0;
		if (!txt) {
			std::cerr << "ERROR: Cannot open txt [" << argv[1] << "].\n";
			return 1;
		}
		do {
			std::getline(txt, text);
			if (txt) {
				if (text[0] != '#') {
					Book temp(text);
					library += temp;
		...
		} while (txt && m_counter < 4);
		library.setObserver(bookAddedObserver);
		// TODO: add the rest of the books from the txt.
		do {
			std::getline(txt, text);
			if (txt && text[0] != '#') {
				library += Book(text);
	...
	auto updatePrices = [usdToCadRate, gbpToCadRate](Book& other) {
		if (other.country() == "US") {
			other.price() *= usdToCadRate;
		}
		else if (other.country() == "UK" && other.year() <= 1999 && other.year() >=
			1990) {
			other.price() *= gbpToCadRate;
		}
	};
	std::cout << "-----------------------------------------\n";
	...
	Movie movies[5];
	{
		// TODO: load 5 movies from the txt "argv[2]".
		// - read one line at a time, and pass it to the Movie constructor
		// - store each movie read into the array "movies"
		// - lines that start with "#" are considered comments and should be ignored
		std::ifstream txt(argv[2]);
		if (!txt) {
			std::cerr << "ERROR: Cannot open txt [" << argv[2] << "].\n";
			return 1;
		}
		std::string strMovies;
		int m_counter = 0;
		do {
			std::getline(txt, strMovies);
			if (txt && strMovies[0] != '#') {
				movies[m_counter] = Movie(strMovies);
				m_counter++;
			}
		} while (txt);
		std::cout << "-----------------------------------------\n";
